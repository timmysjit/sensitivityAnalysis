...
from utils.retarget_dataset import *
import logging
...
def main():
    ...
    train_data = None
    if (CONFIG["PASS_DATA_GENERATION"]):
        items = sorted(os.listdir(CONFIG["DATA_GENERATION_DIR"]))
        train_data = load_dataset_from_dir(CONFIG["DATA_GENERATION_DIR"], items)
    else:
