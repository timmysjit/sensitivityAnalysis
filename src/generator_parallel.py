...
def _run_single(input):

    run_idx = input["run_idx"]
    ...
    normalized_attachments = _shared["normalized_attachments"]
    noise_amplitude = _shared["noise_amplitude"]
    template_pipe_length = input["template_pipe_length"]
    path_points = input["path_points"]


    leak_demand_m3s = input["leak_demand_lps"] / 1000.0
    ...
    try:
        ...
        return {
            ...
            "data" :{
                "runId": run_idx,
                "pipeId": pipe_name,
                "leakDemandLps": input["leak_demand_lps"],
                "leakPositionOnPipe": input["position"],
                "leakLocationWgs84": {
                    "longitude": leak_lon,
                    "latitude": leak_lat,
                },
    ...
    for pipe_name in wn_template.pipe_name_list:
        ...
        for leak_demand_lps in demand_values_lps:
            for position in positions:
                run_idx += 1
                inputs.append({"run_idx" : run_idx, "position" : position, "leak_demand_lps" : leak_demand_lps, "pipe_name" : pipe_name, "template_pipe_length" : float(template_pipe.length), "path_points": path_points})
